            ...
            let applyOperationsDuringSyncScenarios: [TestScenario<Context>] = [
                        ...
                        try await withThrowingTaskGroup(of: Void.self) { group in
                            for keyData in primitiveKeyData {
                                group.addTask {
                                    ...
                                    // TODO: Handle binary protocol conversion like JS test does
                                    ...
                                    try await objectsHelper.processObjectOperationMessageOnChannel(
                                        ...
                                        serial: lexicoTimeserial(seriesId: "aaa", timestamp: 0, counter: 0),
                                        siteCode: "aaa",
                                        state: [objectsHelper.mapSetOp(objectId: "root", key: keyData.key, data: .object(data))]
                                    )
                                }
                            }
                        ...
                        for keyData in primitiveKeyData {
                            if let bytesValue = keyData.data["bytes"] {
                                ...
                                if let stringValue = keyData.data["string"] {
                                    if case .string(let expectedString) = stringValue {
                                    ...
                                    if case .number(let expectedNumber) = numberValue {
                                    ...
                                    if case .bool(let expectedBool) = boolValue {
                ...
                .init(
                    ...
                    action: { ctx in
                        ...
                        let counter = try #require(root.get(key: "counter")?.liveCounterValue)
                        let expectedCounterValue = 1.0 + 1000.0 + 100000.0 + 1000000.0 // sum of passing operations and the initial value
