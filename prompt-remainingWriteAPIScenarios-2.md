...
- started porting the writeAPIScenarios to Swift (with the aid of an LLM); this is done except for 4 which are marked as `disabled: true` and whose implementations have not been properly filled in
...
- If you need to access an object's `objectID` (like the JavaScript tests use private API to do), then cast it to its internal type (e.g. PublicDefaultLiveCounter) and then do `proxied.objectID`.
- If you need to override the method that is used for publishing ObjectMessages (like the JavaScript tests use private API to do), then cast `objects` to PublicDefaultRealtimeObjects and then use `testsOnly_overridePublish(with:`).
